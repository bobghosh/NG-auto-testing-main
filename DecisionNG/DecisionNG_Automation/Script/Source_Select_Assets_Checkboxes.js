  ...
  for(var i =0; i <Tab_Asset.length ; i++)
  {
    ...
    if(AssetType == "Decision")
    {
      ...
      for(var j = 1; j <= Decisioncount.length ; j++)
      {
        ...
        {
          Aliases.browser.pageSapiensDecision.FindElement("//dcn-expandable-selectable-list-item["+j+"]//*[contains(@class,'icon-decision')]//ancestor::div[contains(@class,'dcn-expandable-selectable-list__item__wrapper')]//p-checkbox//div[contains(@class,'ui-chkbox-box')]").Click();
          break;
        }    
      }
    }
    ...
    {     
      ...
      for(var k = 1; k <= Flowscount.length ; k++)
      {
        ...
        if(DecisionName.includes(AssetName))
        {
          Aliases.browser.pageSapiensDecision.FindElement("//dcn-expandable-selectable-list-item["+k+"]//*[contains(@class,'icon-flow')]//ancestor::div[contains(@class,'dcn-expandable-selectable-list__item__wrapper')]//p-checkbox//div[contains(@class,'ui-chkbox-box')]").Click();
          break;
